...
\begin{minipage}{14 cm}
\begin{align*}
	\Gamma 	\Coloneqq		&																\tag{typing environment} \\
									& \cdot															\tag{empty} \\
									& \Gamma, x : \ty										\tag{extended with variable} \\
	\Delta \Coloneqq 	& 																\tag{scope variable environment} \\
									& \cdot															\tag{empty} \\
									& \Delta, s_{var}										\tag{extended with scope variable} \\
	\Sigma \Coloneqq 	& 																\tag{dynamic environment} \\
									& \cdot															\tag{empty} \\
									& \Sigma, s_{loc}										\tag{extended with scope location} \\
									& \Sigma, l := (s_{loc}, \eff)			\tag{extended with instance location} \\
\end{align*}
\label{fig:contexts}
\end{minipage}
}
