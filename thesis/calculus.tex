...
Variables $x$ are simply looked up in the typing environment $\Gamma$ (\textsc{T-Var}).
...
Abstractions $\vabs{x}{\comp}$ are assigned a function type $\tarr{\ty}{\cty}$, by typing the body with $\cty$ after adding $x : \ty$ to the typing environment $\Gamma$ (\textsc{T-Abs}).
...
Values can be assigned a more general type if that type is valid and a supertype of the type of the value (\textsc{T-SubVal}).
...
Finally we give a small-step operation semantics for X.
