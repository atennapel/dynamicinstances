...
\paragraph{Judgments}
There are three kinds of judgments: subtyping (Section~\ref{sec:subtyping}), well-formedness (Section~\ref{
...
The well-formedness judgments assert that scopes used in the types are valid under the scope variable and dynamic environments.
...
\item $\Delta;\Sigma \vdash \cty$ asserts that all the scopes in the computation type $\cty$ are valid under the environments $\Delta$ and $\Sigma$.
\end{itemize}
...
\begin{theorem}[Semantics does valid monotonic extension]
\[
	\mathsf{if}\;
		(\comp \;|\; \Sigma \rightsquigarrow \comp' \;|\; \Sigma')
		\;\land\;
