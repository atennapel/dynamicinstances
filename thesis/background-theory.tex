...
We formalized the system with static instances and have proven the type soundness theorem in the Coq proof assistant.
The formalization follows closely the one for algebraic effects without static instances.
We only formalized the system with handlers with one operation.
Again we use a system that follows the one used by Bauer and Pretnar~\cite{eff2}.
Static instances do not add much complexity compared to algebraic effects.
