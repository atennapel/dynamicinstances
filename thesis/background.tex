...
We will write operation calls as \ident{inst#op(v)} where \ident{inst} is the instance.
Handlers are modified to take an instance parameter as follows \ident{handle#inst(comp) { ... }}.
\\\\
...
Next we will implement references more generally similar to the ones available in Standard ML\cite{standardml}, in our case specialized to \ident{Int}.
