...
The whole computation is wrapped in \ident{handle(s' ->} to actually perform the effects, resulting in \ident{42} without any remaining effects.
\\\\
Using \ident{ref} we can fully emulate multiple mutable references.
We have the added guarantee that the references will not escape their effect scope, they will not escape their corresponding \ident{handle}.
...
In Figure~\ref{fig:vectors} we define the \ident{Vector} algebraic datatype.
...
We wrap this computation in \ident{handle} to handle the effects of the mutable vector.
...
The type system ensures that \ident{handle} actually does encapsulate all effects in its scope.
