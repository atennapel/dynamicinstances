...
substScopeComp x s (New s' e h (Finally y c') z c) =
  New s' e (substScopeHandler x s h) (Finally y (substScopeComp x s c')) z (substScopeComp x s c) 
...
gamma :: Gamma
gamma = GVar GEmpty "True" tBool

term :: Comp
term = App (VAnn (Abs "x" $ Return $ Var "x") (TFun tBool (CType tBool Set.empty))) (Var "True")
...
main = do
  ...
  case tcComp delta gamma term of
    ...
    Right ty -> do
      ...
      putStrLn $ "annotations erased: " ++ (show term')
      stepsShow term'
      return ()
