...
Reserved Notation "Gamma '|-c' t 'in' T" (at level 40).
...
with has_type_comp : context -> comp -> ty -> Prop :=
  | T_Return : forall Gamma v T,
    Gamma |- v in T ->
    Gamma |-c ret v in T
  | T_App : forall Gamma t1 t2 T1 T2,
    Gamma |- t1 in tarr T1 T2 ->
    Gamma |- t2 in T1 ->
    Gamma |-c app t1 t2 in T2
  | T_Do : forall Gamma t1 t2 T1 T2,
    Gamma |-c t1 in T1 ->
    (T1 :: Gamma) |-c t2 in T2 ->
    Gamma |-c do t1 t2 in T2

where "Gamma '|-c' t 'in' T" := (has_type_comp Gamma t T).
...
Lemma typable_closed_length :
  ...
  (forall Gamma t T, Gamma |-c t in T -> closedcomp' (length Gamma) t).
Proof.
...
Corollary typable_closed_empty :
  ...
  (forall Gamma t T, Gamma |-c t in T -> Gamma = nil -> closedcomp t).
Proof.
...
Lemma context_invariance :
  ...
  (forall Gamma t T, Gamma |-c t in T -> forall Gamma', (Gamma ++ Gamma') |-c t in T).
Proof.
...
Lemma shift_typing :
  ...
  (forall Gamma t T,
    Gamma |-c t in T ->
    forall m U,
      (firstn m Gamma ++ U :: skipn m Gamma) |-c shiftcomp' 1 m t in T).
Proof.
...
Lemma substitution_preserves_typing :
  ...
  (forall (t:comp) Gamma Gamma' v T U,
    (Gamma ++ U :: Gamma') |-c t in T ->
    (Gamma ++ Gamma') |- v in U ->
    (Gamma ++ Gamma') |-c substcomp' (length Gamma) v t in T).
Proof.
  ...
  - inversion_try_solve H1.
    apply H with (T := tarr T1 T) (v0 := v1) in H6; auto.
    apply H0 with (T := T1) (v := v1) in H8; auto.
    apply T_App with (T1 := T1); auto.
  - inversion_try_solve H1.
    apply T_Do with (T1 := T1).
    apply H with (U := U); auto.
