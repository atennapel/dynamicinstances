...
typecheckHandleCase ctx t r (o, c) = do
  ...
  CTy t' r' <- typecheckComp (CVar x a : CVar k (TFun b $ CTy t r) : ctx) (openR 1 (Free x) $ openVar k c)
  subTy t' t
  return (e, o, r')
  ...
  case {- trace ("reduceComp " ++ (show c)) -} c of
    ...
    Handle c cr m ->
      case reduceComp c of
        ...
        op@(Op o v) -> Maybe.maybe op (\c -> reduceComp $ openR 1 v $ open (Abs "_" cr) c) $ Map.lookup o m
        Do (Op o v) b ->
    ...
    App "and" "True" -> Return $ Abs "Bool" $ Return $ Bound 0
    App "and" "False" -> Return $ Abs "Bool" "False"

    App "or" "False" -> Return $ Abs "Bool" $ Return $ Bound 0
    App "or" "True" -> Return $ Abs "Bool" "True"
...
ctx = context
  [
    ...
    CVar "not" $ TFun "Bool" (CTy "Bool" Set.empty),
    CVar "and" $ TFun "Bool" (CTy (TFun "Bool" (CTy "Bool" Set.empty)) Set.empty),
    CVar "or" $ TFun "Bool" (CTy (TFun "Bool" (CTy "Bool" Set.empty)) Set.empty),
...
failH :: Expr -> Expr -> Expr
failH v c = EHandle c $ ECase "fail" "x" "k" v End

stateH :: Expr -> Expr
stateH c = EHandle c $
  ERet "x" (EAbs "s" "Bool" "x") $
  ECase "get" "x" "k" (EAbs "s" "Bool" $ eapps "k" ["s", "s"]) $
  ECase "put" "s" "k" (EAbs "_" "Bool" $ eapps "k" ["Unit", "s"]) $
  End

stateF :: Expr -> Expr -> Expr
stateF v c = EApp (stateH c) v

flipConst :: Expr -> Expr -> Expr
flipConst v c = EHandle c $
  ECase "flip" "_" "k" (EApp "k" v) $
  End

flipF :: Expr -> Expr -> Expr
flipF f c = EHandle c $
  ECase "flip" "_" "k" (eapps f [ EApp "k" "False", EApp "k" "True"]) $
  End

term :: Expr
term = flipF (EAbs "x" "Bool" $ EAbs "y" "Bool" $ eapps "or" ["x", "y"]) $
  EDo "b1" (EOp "flip" "Unit") $
  EDo "b2" (EOp "flip" "Unit") $
  eapps "and" ["b1", "b2"]
